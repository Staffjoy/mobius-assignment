...
while true; do
    ...
    # See what's deployed
    version=`aws elasticbeanstalk describe-environments --application-name "staffjoy-mobius" --environment-name "mobius-prod" --query "Environments[*].VersionLabel" --output text`
    status=`aws elasticbeanstalk describe-environments --application-name "staffjoy-mobius" --environment-name "mobius-prod" --query "Environments[*].Status" --output text`
